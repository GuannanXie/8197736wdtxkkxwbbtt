...
For the data-driven simulation part, we also quote an additional $\sim$5\% difference to capture the difference between the pure Hijing and data-driven simulation relay on Hijing when we validating the packages. This additional source is kind of conservative since we already using the 50\% and 150\% efficiency method to calculate the systematic source. But it still worth to be conservative. Note here, just for plotting, this $\sim$5\% uncertainties was included and shown with "vertex correction" in the Fig.~\ref{sysErr_0_10} to Fig.~\ref{sysErr_0_80}, as you can see, this is no actual vertex correction for 0-10\% and 10-20\%.
